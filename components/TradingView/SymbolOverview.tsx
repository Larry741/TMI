...
import {
  ...
  ScaleMode,
  ScalePosition
} from "@/interface/tradingview";
import { createId } from "@/utils/createIds";
...
const defaultSymbols = [["TVC:GOLD|1D"], ["TVC:SILVER|1D"], ["TVC:UKOIL|1D"]];
...
const SymbolOverview: React.FC<SymbolOverviewProps> = ({
  ...
  const altSymbols: string[][] = [];

  symbols.forEach((symbolArr, idx) => {
    if (symbolArr[0].split("|")[0] === initSymbol) {
      altSymbols.unshift(symbols[idx]);
    } else {
      altSymbols.push(symbols[idx]);
    }
  });
